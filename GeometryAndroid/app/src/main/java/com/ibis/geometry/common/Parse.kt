...
fun ParseContext.parseLine(init: String): Reactive<Drawable> {
    ...
    val (res, next) = parseSum(value) ?: error("Parsing failed: $value")
...
fun<T> parseInfix(
    ...
    var (res, line) = parser(init) ?: return null
    line = line.trimStart()
    while (true) {
        val op = ops.firstOrNull { line.startsWith(it.first) } ?: return res to line
        val (next, nextLine) = parser(line.substring(op.first.length).trimStart()) ?: return res to line
...
fun ParseContext.parseApp(line: String): Pair<Reactive<Geometric>, String>? = when {
    ...
    line.startsWith("(") ->
        parseSum(line.substring(1).trimStart())?.mapSnd {
            check(it.startsWith(")")) { "Expected ')'" }
            it.substring(1)
        }
    line.startsWith("#") ->
        parseApp(line.substring(1).trimStart())?.let {
            check(it.first is Static && (it.first as Static<*>).value is Complex) { "Expected point" }
...
fun ParseContext.parseAtom(line: String): Pair<Reactive<Geometric>, String>? =
    parseWord(line).let { (word, next) ->
        if (next.startsWith("(")) {
            val (args, next2) = parseInfix(next.substring(1), { parseSum(it)?.mapFst(::listOf) },
                "," to { a, b -> a + b })
...
fun parseWord(line: String): Pair<String, String> =
    line.takeWhile { it.isLetterOrDigit() || it in "_'{}" }.let {

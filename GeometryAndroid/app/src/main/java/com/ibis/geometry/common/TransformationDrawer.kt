...
class TransformationDrawer<T : Drawer>(val drawer: T, private val transformation: TransformationState, val size: Size) : Drawer {
    override val zoom get() = transformation.zoom
    ...
    override fun angle(center: Offset, from: Float, to: Float, style: Style) = drawer.angle(get(center), from + transformation.rotation, to + transformation.rotation, style)
}